...
def map_any_case_to_lower(any_case_input: list) -> dict:
    """Map a list of strings values, given as input in any kind of casing combination,
        to a lower case corresponding key.
...
class LanguagesRankingExtractor(LanguagesStatsExtractor):
    ...
    def __init__(
        ...
        columns_selection_criteria=None,
        exclusion_list=None,
        entries_merge_list=None,
        prefix_to_remove="",
    ):
        """
    ...
    def compute_language_proficiency_ranking(self, ignore_case=True, ascending=False) -> pd.Series:
        ...
        for to_be_excluded in self.__exclusion_list:
            if ignore_case and (to_be_excluded.lower() in proficiencies_lower_to_original_map.keys()):
                df_proficiencies = drop_columns_from_map(
                    df_proficiencies, proficiencies_lower_to_original_map, to_be_excluded.lower()
                )
...
class LanguagesProficienciesPercentages(LanguagesStatsExtractor):
    ...
    def get_stats(self) -> dict:
        ...
        """
        return {
            "number respondents": self.get_data_source().shape[1],
    ...
    def joint_share(
        self, languages: list, unison: bool = False, platform_key: str = "PlatformWorkedWith", platform: str = None
    ) -> float:
        ...
        if unison:
            share_sum = self.__lre.get_data_source()[
                ((self.__lre.get_data_source()[languages] != 0).all(axis=1)) & platform_condition
            ].shape[0]
        else:
            share_sum = self.__lre.get_data_source()[
                ((self.__lre.get_data_source()[languages] != 0).any(axis=1)) & platform_condition
            ].shape[0]
    ...
    def exclusive_share(self, ref_language: str, excluded_languages: list, platform: str = None) -> float:
        ...
        ref_share = self.__lre.get_data_source()[
            reference_language_mask
            & (self.__lre.get_data_source()[excluded_languages] == 0).all(axis=1)
            & platform_condition
        ].shape[0]
    ...
    def intersection_percentage(self, language_1: str, language_2: str, overall: bool = False) -> float:
        ...
        overlap_count = self.__lre.get_data_source()[
            (self.__lre.get_data_source()[language_1] != 0) & (self.__lre.get_data_source()[language_2] != 0)
        ].shape[0]
    ...
    def difference_percentage(self, language_1: str, language_2: str, union_relative: bool = False) -> float:
        ...
        """
        if union_relative:
            base_count = self.__lre.get_data_source()[
                (self.__lre.get_data_source()[language_1] != 0) & (self.__lre.get_data_source()[language_2] != 0)
            ].shape[0]
        else:
            base_count = self.__lre.get_data_source()[self.__lre.get_data_source()[language_1] != 0].shape[0]

        difference_count = self.__lre.get_data_source()[
            (self.__lre.get_data_source()[language_1] != 0) & (self.__lre.get_data_source()[language_2] == 0)
        ].shape[0]
